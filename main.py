...
global firebaseFile
global dbURL, dbHost, dbUser, dbPassword, dbSchema
...
configs = Properties()
with open("allCreds/DBCreds.properties", "rb") as config_file: configs.load(config_file)
...
# Getting Firebase crdentials from the properties file
firebase_configs = Properties()
with open("allCreds/firebaseCreds.properties", "rb") as firebase_config_file: firebase_configs.load(firebase_config_file)
firebaseFile = firebase_configs.get("firebaseFile").data
dbURL = firebase_configs.get("dbURL").data

# Getting DB crdentials from the properties file
smtp_configs = Properties()
with open("allCreds/smtpCreds.properties", "rb") as smtp_config_file: smtp_configs.load(smtp_config_file)
...
try:
    ...
    print(
        "Error connecting to Firebase, Please check the allCreds/firebaseCreds.properties file"
    )
...
def user():
    ...
    mycursor = conn.cursor()
    sql = (
        "SELECT balance from balances where username = " + "'" + session["email"] + "'"
    )
    mycursor.execute(sql)
    vcoins = mycursor.fetchone()
    try: session["vcoins"] = vcoins[0]
    except: session["vcoins"] = 0
    ...
    return render_template(
        "user.html",
        vcoins=session["vcoins"],
        tasks=tasks,
        body=body,
        profileSeed=session["email"].split("@")[0],
    )
...
def myHoldings():
    ...
    sql = (
        ...
        WHERE holdings.username = "
        + "'"
        + session["email"]
        + "' and holdings.coin_amount > 0"
    )
    ...
    for k in tasks:
        session["netWorth"] = (
            session["netWorth"]
            + float(k[3]) * getCoinPriceFromId(k[4])
            + session["vcoins"]
        )
        body += (
            "<div class='card'> <img src='"
            ...
            + " ("
            + k[6]
            + ") </h3> <h4>Total Value(INR):- "
            + str(round(float(k[3]), 2) * getCoinPriceFromId(k[4]))
            + "</h4> <a href=/buy/"
            + str(k[4])
            + "> <button class='buy'> Buy More </button> </a> <a href=/sell/"
            + str(k[4])
            + "> <button class='sell'> Sell </button> </a> </div> <br> <br>"
        )
    ...
    return render_template(
        ...
        body=body,
        profileSeed=session["email"].split("@")[0],
        netWorth=round(session["netWorth"], 2),
    )
...
def signup():
    ...
    if request.method == "POST":
        ...
        if createEmail != None:
            ...
            session["createPassword"] = hashlib.sha256(
                session["createPassword"].encode()
            )
            ...
            if createEmail != None and createPassword != None:
                if "@" and "," in firebaseCreateEmail:
                    ...
                    else: return accountExistsSignup()
                else: return invalidEmailSignup()
            else:
                return invalidEmailSignup()

    return render_template("signup.html")

# set up redirects as function for internal execution
invalidEmailSignup = lambda: redirect("/signup#invalidEmailModal")
accountExistsSignup = lambda: redirect("/signup#haveAccountModal")
...
def changePassword():
    ...
    if oldPassword and newPassword != None:
        ...
        else: return redirect("/changePassword#incorrectPasswordModal")
    return render_template("changePassword.html")

generatePin = lambda: random.SystemRandom().randint(1000, 9999)
    ...
    if session["emailFlag"] == 1:
        ...
        try:
            s.sendmail(
                smtpEmail,
                session["createEmail"],
                "Subject: Email Verification pin for CryptoPlay \n\n Your CryptoPlay pin is: "
                + str(session["pin"])
    ...
    if request.method == "POST":
        ...
        if pin != None:
            ...
            if pin == str(session["pin"]):
                session["tries"] = 0
                ref.child("Users/" + session["firebaseCreateEmail"]).set(
                    session["createPassword"]
                )
                ...
                except: return redirect("/verifyEmail#refreshModal")
...
def buyOrder(id, inrVal):
    ...
    if float(inrVal) > session["balance"]:
        ...
        if len(result) <= 0:
            ...
            sql = (
                "insert into holdings (holding_id, username, coin_id, coin_amount) Values (default, '"
                + session["email"]
                + "', "
                + id
                + ", "
            ...
            sql = (
                "update holdings set coin_amount = coin_amount + "
                + str(session["coinAmt"])
                + " where username = '"
                + session["email"]
                + "' and coin_id = "
        ...
        mycursor = conn.cursor()
        sql = (
            "insert into transactions (transaction_id, username, value, coin_id, coin_amount, transaction_type) Values (default, '"
            + session["email"]
            + "', "
            + inrVal
            + ", "
...
def transactions():
    ...
    for transaction in transactions:
        ...
        if transaction[5] == "buy":
            body += (
                "<div class='card' style='color:green'> <h3> You bought "
                + str(transaction[4])
                + " "
                + coin
                + " for ₹"
            ...
            body += (
                "<div class='card' style='color:red'> <h3> You sold "
                + str(transaction[4])
                + " "
                + coin
                + " for ₹"
